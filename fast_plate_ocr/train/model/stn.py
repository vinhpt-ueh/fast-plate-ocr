...
# Create the main model with STN
def create_model(input_shape):
    inputs = layers.Input(shape=input_shape)
    localization_net = create_localization_net(input_shape)
    stn = SpatialTransformer(localization_net)(inputs)
    # Continue with the rest of your model
    x = layers.Conv2D(32, kernel_size=3, activation="relu")(stn)
    x = layers.MaxPooling2D(pool_size=2)(x)
    x = layers.Flatten()(x)
